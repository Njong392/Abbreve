...
const Form = () => {
    ...
    const fetchData = (event) => {
        ...
        fetch('https://njong392.github.io/data/server.json')
        .then(response => {
            if(!response.ok){
    ...
    return(
        <div className=" bg-dark py-12 px-14">
            <section className="block justify-center pb-16 md:flex">
                ...
                <div>
                   <form className="block md:flex items-center gap-3">
                        <div className="bg-ash h-11 rounded-full flex items-center p-3">
                            ...
                            <input type="search" placeholder="Search slang full meaning..." className="flex-1 w-1/2 h-11 rounded-full ml-2 border-none outline-none text-gray text-lg" onChange={handleChange} />

                        </div>

                        <button className='bg-purple text-ash text-sm font-bold rounded-xl hover:scale-110 outline-none mt-2' onClick={fetchData}>Submit</button>
                   </form>
